...
- a collision between the robot and an obstacle { "collision" : "false", "move": "moveForward"}
    ...
    public void  handleInfo(JSONObject infoJson) {
        ...
        if( infoJson.has("endmove") || infoJson.has("sonarName") || infoJson.has("collision"))  SyncHandleInfo(infoJson);
        else if( infoJson.has("robotcmd") )  handleRobotcmd(infoJson);
    }

    public synchronized void SyncHandleInfo(JSONObject infoJson) {
    ...
    private void handleRobotcmd(JSONObject infoJson) {

        String robotcmd = (String) infoJson.get("robotcmd");
        if(robotcmd.equals("RESUME")) {
            robotBehaviorLogic.doBoundaryGoon();
        } else if(robotcmd.equals("STOP")) {
            robotBehaviorLogic.doBoundaryStopRobot();
        } else {
        }
    }
